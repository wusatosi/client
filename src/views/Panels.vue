<template>
  ...
  <div id="panels">
    ...
    <pb v-bind:panels="this.panels" v-bind:drawerFactory="this.drawerFactory" ref="pb"/>
  </div>
</template>
<script lang="ts">
import Vue from "vue";
...
import {SizedPanel} from "@/utils/PanelTypes";
import {DefaultLinkingLineDrawer} from "@/utils/DefaultLinkingLineDrawer";
...
const drawerFactory = () => new DefaultLinkingLineDrawer()
...
export default Vue.extend({
  ...
  methods: {
    _panelPosUpdateRequest(panel: SizedPanel, event: PanelPositionUpdateEvent) {
      const posRef = event.self.position;
      posRef.top = event.newPos.top;
      posRef.left = event.newPos.left;
      // cannot get PanelBackground.reRenderRelated through typescript typing system
      // as it is of type Vue / Element & reRenderRelated is defined in methods()
      // TODO: use class style syntax to make typescript happy
      // @typescript-eslint/no-explicit-any
      (this.$refs.pb as any).reRenderRelated(panel);
    }
